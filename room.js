...
mod.extend = function(){
    ...
    Object.defineProperties(Room.prototype, {
        ...
        'boostQueue': {
            configurable: true,
            get: function() {
                if( _.isUndefined(this.memory.boostQueue) ) {
                    this.memory.boostQueue = [];
                }
    ...
    Room.prototype.clearReactionOrders = function() {
  console.log('clearing lab reactions');
  let labs = this.find(FIND_MY_STRUCTURES, {
      filter: (structure) => ( structure.structureType == STRUCTURE_LAB )
  });
  labs.forEach((l) => {
    console.log(l.id);
    this.cancelReactionOrder(l.id)
  })
}
Room.prototype.clearLabContents = function() {
  console.log('clearing lab contents');
  let labs = this.find(FIND_MY_STRUCTURES, {
      filter: (structure) => ( structure.structureType == STRUCTURE_LAB )
  });
  labs.forEach((l) => {
    console.log(l.id);
    this.placeOrder(l.id, l.mineralType, -l.mineralAmount);
  })
}
Room.prototype.fillLabEnergy = function() {
  console.log('filling lab energy');
  let labs = this.find(FIND_MY_STRUCTURES, {
      filter: (structure) => ( structure.structureType == STRUCTURE_LAB )
  });
  labs.forEach((l) => {
    console.log(l.id);
    this.setStore(l.id, 'energy', 2000);
  })
}
Room.prototype.fillLabMinerals = function() {
  console.log('filling lab Minerals');
  let labs = this.find(FIND_MY_STRUCTURES, {
      filter: (structure) => ( structure.structureType == STRUCTURE_LAB )
  });
  var minerals = ['XGHO2', 'XLHO2', 'XZH2O', 'XZHO2', 'XUH2O', 'XKHO2'];
  minerals.forEach((mineral, index) => {
    this.setStore(labs[index].id, mineral, 3000);
  })
